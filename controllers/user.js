...
exports.findid = async (req, res, next) => {
  ...
  try {
    ...
    if (user.verificationExpiresAt < new Date()) {
      return res
        .status(400)
        .json({ message: "유효 시간이 만료되었습니다. 다시 시도해주세요." });
    }
    ...
    if (user.name === name) {
      ...
      return res
        .status(200)
        .json({ message: "아이디 찾기 성공", foundId: user.id });
    } else {
...
exports.resetpw = async (req, res, next) => {
  ...
  if (newPassword !== newPasswordCheck) {
    return res
      .status(400)
      .json({ message: "비밀번호가 서로 일치하지 않습니다. 다시 입력하세요." });
  }
  ...
  try {
    ...
    if (isSamePassword) {
      return res.status(400).json({
        message:
          "새 비밀번호가 기존 비밀번호와 동일합니다. 다른 비밀번호를 입력하세요.",
      });
    }
...
exports.pageIdSuccess = (req, res) => {
  try {
    res
      .status(200)
      .json({ message: "아이디찾기페이지 조회 성공", foundId: req.params.id }); // 상태 코드 200과 메시지를 JSON 형식으로 전송
  } catch (error) {
    res.status(500).json({ message: "500 서버 오류" });
  }
};
...
//비밀번호 재설정 페이지
exports.pageResetPw = async (req, res, next) => {
  //쿠키 설정 수정0930
